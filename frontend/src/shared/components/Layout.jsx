...
import { getTrainingDevelopmentNavChildren } from '@/features/training-development/navConfig';
import { getHighSkillRetentionNavChildren } from '@/features/high-skill-talent-retention/navConfig';
import { getEmployeeLifecycleNavChildren } from '@/features/employee-lifecycle-management/navConfig';
import { getWorkforceIntelNavChildren } from '@/features/workforce-intelligence/navConfig';
import { getCostOptimizationNavChildren } from '@/features/cost-optimization-automation/navConfig';
import { getEmployeeSatisfactionEngagementNavChildren } from '@/features/employee-satisfaction-engagement/navConfig';
import PlacementHeaderFilters from '@/features/smart-hiring/components/hiring/PlacementHeaderFilters';
...
import { resolveSmartHiringNavVariant } from '@/shared/config/smartHiringNav';
import {
  Sheet,
  SheetContent,
  SheetHeader,
...
const Layout = ({ children }) => {
  ...
  return (
    ...
    >
      ...
      {SMART_HIRING_ONLY ? (
        <aside
          className={`lg:hidden fixed inset-y-0 left-0 z-50 w-[min(100vw-3rem,18.75rem)] max-w-[300px] transform transition-transform duration-300 ${
        ...
        >
          <div className="flex items-center gap-4 min-w-0 flex-1">
            ...
            {!isSmartHiringDashboard ? (
              <h1 className="text-base sm:text-lg font-semibold text-slate-900 font-['Outfit'] truncate max-w-[40vw] sm:max-w-none">
                {pageTitle}
              </h1>
            ) : (
              <h1 className="lg:hidden text-base font-semibold text-slate-900 font-['Outfit'] truncate">
                Smart Hiring
              </h1>
            )}
