...
const DEFAULT_ROW = { skill_name: '', demand_count: 0, priority: 'MEDIUM' };

const ProjectDemandsPage = () => {
  ...
  const loadSkillDemands = async (projectId) => {
    if (!projectId) return;
    setRowsLoading(true);
    try {
      const res = await projectsApi.listSkillDemands(projectId);
      const list = res.data || [];
      if (list.length === 0) {
        setRows([{ ...DEFAULT_ROW }]);
      } else {
      ...
      toast.error('Failed to load skill demands');
    } finally {
  ...
  useEffect(() => {
    if (selectedProjectId) {
      loadSkillDemands(selectedProjectId);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selectedProjectId]);
  ...
  const handleCreateProject = async () => {
    ...
    try {
      ...
      setNewProjectDesc('');
      await loadProjects();
      setSelectedProjectId(res.data?.id || selectedProjectId);
    } catch (e) {
  ...
  const handleUpsertDemands = async () => {
    ...
    try {
      ...
      await projectsApi.upsertSkillDemands(selectedProjectId, {
        rows: cleaned,
        mode: 'upsert',
      });

      toast.success('Skill demands saved');
      await loadSkillDemands(selectedProjectId);
    } catch (e) {
      toast.error(e?.response?.data?.detail || 'Failed to save skill demands');
    } finally {
      setSaving(false);
    }
  };
  ...
  const downloadTemplate = () => {
    const csv = ['skill_name,demand_count,priority', 'Python,50,HIGH'].join('\n');
    const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = 'project_skill_demands_template.csv';
  ...
  const parseDemandsFromLines = (lines) => {
    const headers = lines[0].split(',').map((h) => h.trim().toLowerCase());
    const col = (name) => headers.indexOf(name);
    const required = ['skill_name', 'demand_count'];
    for (const r of required) {
      if (col(r) < 0) {
        throw new Error(`Missing required CSV column: ${r}`);
      }
    }
  ...
  const executeImportFromLines = async (lines, dryRun) => {
    ...
    const { payloadRows, rawByRowNumber } = parseDemandsFromLines(lines);
    const res = await projectsApi.bulkImportSkillDemands(selectedProjectId, {
      mode: importMode,
      dry_run: dryRun,
      rows: payloadRows,
    });
    ...
    try {
      ...
      if (isPreview) {
        ...
        if (failures.length > 0) {
          ...
          const a = document.createElement('a');
          a.href = url;
          a.download = 'project_skill_demand_import_failures.csv';
          document.body.appendChild(a);
          a.click();
          document.body.removeChild(a);
          URL.revokeObjectURL(url);
      ...
      toast.error(e?.message || e?.response?.data?.detail || 'Failed to process CSV');
    } finally {
      event.target.value = '';
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-2xl font-bold text-slate-900" style={{ fontFamily: 'Outfit' }}>Project Demands (M4)</h1>
          <p className="text-slate-600">Define skill demand per project to drive workforce forecasting and resource optimization.</p>
        </div>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Used for project-sourced demand in M3/M4.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              <Label>Project Name</Label>
              <Input value={newProjectName} onChange={(e) => setNewProjectName(e.target.value)} placeholder="e.g., Q2 Product Launch" />
            </div>
            <div className="space-y-2">
              <Label>Description (optional)</Label>
              <Input value={newProjectDesc} onChange={(e) => setNewProjectDesc(e.target.value)} placeholder="Brief details" />
            </div>
          </div>
          <Button className="bg-indigo-600 hover:bg-indigo-700" onClick={handleCreateProject} disabled={saving || !newProjectName.trim()}>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Select a project to edit its skill demand rows.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          {isNoProjects ? (
            <div className="text-slate-500">No projects yet. Create one above to start.</div>
          ) : (
      ...
      <Card>
        <CardHeader>
          <CardTitle>Skill Demand Rows</CardTitle>
          <CardDescription>{rowsLoading ? 'Loading...' : 'Upsert demand_count and priority per skill.'}</CardDescription>
        </CardHeader>
        <CardContent>
          {isNoProjects ? (
            <div className="text-slate-500">Create a project to edit demands.</div>
          ) : (
            <>
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead>Demand Count</TableHead>
                    ...
                    {rows.map((r, idx) => (
                      <TableRow key={idx}>
                        ...
                        <TableCell className="min-w-[170px]">
                          <Input
                            type="number"
                            value={r.demand_count}
                            onChange={(e) => {
                              const val = Number(e.target.value);
                              setRows((prev) => prev.map((x, i) => (i === idx ? { ...x, demand_count: isNaN(val) ? 0 : val } : x)));
                            }}
                          />
                        </TableCell>
                        <TableCell className="min-w-[140px]">
                          <Input
                            ...
                            onChange={(e) => {
                              const val = e.target.value;
                              setRows((prev) => prev.map((x, i) => (i === idx ? { ...x, priority: val } : x)));
                            }}
              ...
              <div className="flex items-center gap-3 mt-4 flex-wrap">
                ...
                <Button className="bg-indigo-600 hover:bg-indigo-700" onClick={handleUpsertDemands} disabled={saving || rowsLoading}>
                  {saving ? 'Saving...' : 'Save Demands'}
                </Button>
                <Badge variant="secondary">Tip: M3/M4 use project demands if any exist</Badge>
              </div>
      ...
      <Dialog open={previewOpen} onOpenChange={setPreviewOpen}>
        <DialogContent className="max-w-4xl">
          <DialogHeader>
            <DialogTitle>Dry-Run Import Preview (Project Skill Demands)</DialogTitle>
          </DialogHeader>
...
export default ProjectDemandsPage;
