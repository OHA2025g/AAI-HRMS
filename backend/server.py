...
PHASE1_PERMISSIONS = {
    ...
    "recruiter": {"employees_read", "skills_read", "kpi_read", "lifecycle_write"},
    ...
    "hr_viewer": {"employees_read", "skills_read", "kpi_read"},
}
...
async def update_candidate(
    ...
    if headline is not None:
        update_doc["headline"] = headline
    if total_experience_years is not None:
        update_doc["total_experience_years"] = total_experience_years
    if skills is not None:
        skill_list = [s.strip() for s in skills.split(",") if s.strip()]
        update_doc["skills"] = [{"skill_name": s, "proficiency": None} for s in skill_list]
...
async def match_candidates_to_job(job_id: str, current_user: dict = Depends(get_current_user)):
    ...
    # Ingest from enabled external connectors (LinkedIn + Naukri) and canonicalize with dedup.
    ...
    candidates = await db.candidates.find({}, {"_id": 0}).to_list(500)
    candidates = [c for c in candidates if c["id"] not in applied_ids]
